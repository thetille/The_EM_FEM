...
for f = f_list(1:10)%0.75*10^9
    ...
    if mod(f*10^-9,1) < 0.0001
        if mode == 1
            ...
            filename = sprintf('cst_res_flat/e-field (f=%.0f) [1(1)].txt',f*10^-9);
        end
    elseif mod(f*10^-9,0.1) > 0.01
        if mode == 1
            filename = sprintf('cst_res2/e-field (f=%.2f) [1].txt',f*10^-9);
        else
            filename = sprintf('cst_res_flat/e-field (f=%.2f) [1(1)].txt',f*10^-9);
        end
    else
        if mode == 1
            filename = sprintf('cst_res2/e-field (f=%.1f) [1].txt',f*10^-9);
        else
            filename = sprintf('cst_res_flat/e-field (f=%.1f) [1(1)].txt',f*10^-9);
        end
    end
    delimiterIn = ' ';
    headerlinesIn = 2;
