...
%     isTmp_FF = fsVtr'*fsVtr; % The direction adjusted for the uElMtx_FF matrix
    ...
    meRes_EE(idxRes_EE + (1:incRes_EE) - 1) = isTmp_EE(:).*AElMtx_EE(:); % resulting data for eMtx
    msRes_EE(idxRes_EE + (1:incRes_EE) - 1) = isTmp_EE(:).*BElMtx_EE(:); % resultingdata for sMtx
...
AMtx = sparse(irRes_EE, icRes_EE, meRes_EE, edNumGlo, edNumGlo); % M^{epsilon}
BMtx = sparse(irRes_EE, icRes_EE, msRes_EE, edNumGlo, edNumGlo); % M^{sigma}
