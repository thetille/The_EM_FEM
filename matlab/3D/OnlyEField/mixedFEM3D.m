...
% Read mesh
file_list = ["cylinder_waveguide2", "waveguide_model3","mesh_cylinder_R0", "waveguide_model3 - simple"];
load(file_list(2))
...
[eigTmp, eigIdx_sort] = sort(real(eigVal)); % sorts igenvalues after imag amplitude
eigVal     = eigVal(eigIdx_sort); % w/c0c, selects the largest eigenvalues
eigVtr_int = eigVtr_int(:,eigIdx_sort);
fr = c0*sqrt(eigVal)/(2*pi); % eigenfrequency
...
eFld_all(edIdx_int,:) = eigVtr_int(:,:); % sets all interieor edges to eigenvector value
