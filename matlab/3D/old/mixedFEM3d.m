...
edIdx_pec = ElementDatabase_Get('edges', ed2no_pec);
noIdx_pec = unique(ed2no_pec(:))';
% Find all edges in the database
edNum_all = ElementDatabase_Cardinal('edges');
faNum_all = ElementDatabase_Cardinal('faces');
edIdx_all = 1:edNum_all;
noIdx_all = 1:size(no2xyz,2);
% Compute the interior edges
edIdx_int = setdiff(edIdx_all, edIdx_pec);
noIdx_int = setdiff(noIdx_all, noIdx_pec);
% Assemble global matrices
[eMtx, sMtx, cMtx, uMtx] = ...
    Fem_Assemble(no2xyz, el2no, el2ma, ma2er, ma2si);
eMtx_int = eMtx(edIdx_int,edIdx_int); % mass matrix with
permittivity
sMtx_int = sMtx(edIdx_int,edIdx_int); % mass matrix with
conductivity
cMtx_int = cMtx(:,edIdx_int); % curl matrix
uMtx_int = uMtx; % mass matrix with unity
coefficient
...
elseif strcmp(solver, 'sparse')
    [eigVtr_int, eigVal] = eigs(aMtx, 0.5*(bMtx+bMtx'), 30, ...
        1i*(19.5+1i));
else
...
for mIdx = mVtr
    ...
    for dIdx = 0:1
        for edIdx = 1:size(ed2no_pec,2)
            ...
            xyzTmp = no2xyz(:,noTmp);
            plot3(dIdx*dVal + xyzTmp(1,:), xyzTmp(2,:), ...
                xyzTmp(3,:),'Color', 0.5*[1 1 1]), hold on
        end
        if dIdx == 0
            exViz = real(exFld_all(:,mIdx).');
            eyViz = real(eyFld_all(:,mIdx).');
            ezViz = real(ezFld_all(:,mIdx).');
            quiver3(dIdx*dVal + no2xyz(1,:), no2xyz(2,:), ...
                no2xyz(3,:), ...
